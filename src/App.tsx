...
function MatchProgressPanel({ progress }: {
  ...
  const { phase, pct, chunkIdx, totalChunks, shortStart, shortEnd, segmentsFound, startTime } = progress;
...
export default function App() {
  ...
  // Live matching progress (SSE stream from /api/match)
  ...
  // ---------------------------------------------------------------------------
  async function handleReattach(jobId: string) {
  ...
  }, [isProcessingRef, refJobId, isProcessingTarget, targetJobId]);
  ...
  const handleRunAnalysis = async () => {
    ...
    if (processMode === 'server') {
      ...
      try {
          ...
          for (const part of parts) {
            ...
            if (payload.type === 'progress') {
              setMatchProgress({
                phase: payload.phase,
                pct: payload.pct,
                chunkIdx: payload.chunkIdx,
                totalChunks: payload.totalChunks,
                shortStart: payload.shortStart,
                shortEnd: payload.shortEnd,
                segmentsFound: payload.segmentsFound,
        ...
        if (!data) throw new Error('Match stream ended without result');
