...
function JobCard({
  ...
  // Estimate time remaining for running jobs
  let etaStr: string | null = null;
  if (isRunning && job.startedAt && job.processedFrames > 0 && job.totalFrames > 0) {
  ...
  return (
    <div className={`rounded-xl border p-3 space-y-2.5 transition-colors ${
      ...
      {job.status === 'completed' && job.totalFrames > 0 && (
